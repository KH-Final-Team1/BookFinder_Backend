...
public class JwtAuthenticationFilterTest {
  ...
  public void success_OnAuthenticatedAuthorization_WithValidAuthorization() throws Exception {
    // Given: 유효한 Jwt 토큰이 주어진다.
    String validJwtToken = "validToken";
    // And: User가 주어진다.
    User mockUser = MockUser.getMockUser();
    // And: JwtService가 validJwtToken을 반환하도록 Mocking한다.
    when(jwtService.extractAccessToken(any())).thenReturn(validJwtToken);
    // And: JwtService가 true를 반환하도록 Mocking한다.
    when(jwtService.validateToken(validJwtToken)).thenReturn(true);
    // And: JwtService가 mockUser의 email을 반환하도록 Mocking한다.
    when(jwtService.extractEmail(validJwtToken)).thenReturn(mockUser.getEmail());
  ...
  public void fail_OnAuthenticatedAuthorization_WithInvalidJwtSignature() throws Exception {
    // Given: 유효하지 않은 Jwt 토큰이 주어진다.
    String invalidJwtToken = "invalidToken";
    // And: JwtService가 validJwtToken을 반환하도록 Mocking한다.
    when(jwtService.extractAccessToken(any())).thenReturn(invalidJwtToken);
    // And: JwtService가 Exception을 발생하도록 Mocking한다.
    when(jwtService.validateToken(invalidJwtToken)).thenThrow(new JwtException(Message.INVALID_JWT_SIGNATURE));
  ...
  public void fail_OnAuthenticatedAuthorization_WithInvalidJwt() throws Exception {
    ...
    when(jwtService.extractAccessToken(any())).thenReturn(invalidJwtToken);
    // And: JwtService가 Exception을 발생하도록 Mocking한다.
    when(jwtService.validateToken(invalidJwtToken)).thenThrow(new JwtException(Message.INVALID_ACCESS_TOKEN));
  ...
  public void fail_OnAuthenticatedAuthorization_WithExpiredToken() throws Exception {
    ...
    when(jwtService.extractAccessToken(any())).thenReturn(invalidJwtToken);
    // And: JwtService가 Exception을 발생하도록 Mocking한다.
    when(jwtService.validateToken(invalidJwtToken)).thenThrow(new JwtException(Message.EXPIRED_ACCESS_TOKEN));
